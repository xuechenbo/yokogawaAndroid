...
//                    .allowMainThreadQueries()//主线程操作
    ...
    public AAADao getUserDao() {
        return mDb.getUserDao();
    }

    public ExamiDAO getexamDao() {
        return mDb.getexamDao();
    }
    ...
    private static final Migration MIGRATION_4_5 = new Migration(4, 5) {
        ...
        public void migrate(SupportSQLiteDatabase database) {
            // 创建临时表
            database.execSQL(
                    "CREATE TABLE AAAA_new (userid TEXT, username TEXT, last_update INTEGER, PRIMARY KEY(id))");
            // 拷贝数据
            database.execSQL(
                    "INSERT INTO AAAA_new (userid, username, last_update) SELECT userid, username, last_update FROM users");
            // 删除老的表
